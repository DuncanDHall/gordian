...
def cache_result(duration: float = 0.1):
    ...
    def caching_decorator(func: Callable[[Any], Any]):
        ...
        def wrapper(*args, **kwargs):
            if func in Interpreter.method_cache and Interpreter.method_cache[func, args].expire_time >= Interpreter.ai.time:
                return Interpreter.method_cache[func, args].value
            else:
                result = func(*args, **kwargs)
                Interpreter.method_cache[func, args] = CachedResult(Interpreter.ai.time + duration, result)
                return result
        return wrapper
...
class Interpreter:
    ...
    def mineral_fields(self):
        return self.ai.mineral_field

    @property
    @cache_result(3600.0)
    def expansion_locations(self):
        return self.ai.expansion_locations_list

    @property
    def units(self):
        return self.ai.units
